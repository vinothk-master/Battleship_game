...
class Game_Matrix:
    def __init__(self, root, row, column, matrix_name, grid_instance):
        ...
        if matrix_name == "Matrix A":
            ...
            self.matrixB = [[None for _ in range(10)] for _ in range(10)]
        self.sequence = 1
        self.clicked = False
      #  self.grid_a_instance = grid_a_instance
        P = tk.Label(root, text="PLAYER")
        P.grid(row=0, column=2, padx = 10, pady=10)
        comp = tk.Label(root, text="COMPUTER")
        comp.grid(row=0, column=4, padx = 10, pady=10)

        #if matrix_name =="Matrix B":
        for r in range(10):
            for c in range(10):
...
class Setup_Matrix:
    def __init__(self, root, player_ship_positions):
    ...
    def computer(self):
        ...
        Game_start(self.matrix_A, self.matrix_B, self.root, self.player_ship_positions)

class Game_start(Game_Matrix):
    def __init__(self, matrix_A, matrix_B, root, player_ship_positions):
        self.root = root
                ...
                if self.target in self.player_ship_positions:
                    ...
                    for deck1 in self.player_ships_at_war:
                       if self.target in self.player_ships_at_war[deck1]["Ship"]:
                           ...
                           self.matrix_A.matrixA[self.random_row][self.random_col].config(text="X", bg = str(self.computer_ships_at_war[deck1]["color"]))
                    ...
                    self.matrix_A.matrixA[self.random_row][self.random_col].config(text="O", bg = "blue")
                    self.messsage_label.config(text="MISS")
